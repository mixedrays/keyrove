...
follow it. Drag the corner of the panel to narrow it — or narrow the window —
then arrow around: the rows fold as the columns do.
...
time it is read. `getComputedStyle` resolves one property on one element once per
keypress, which costs nothing you would notice.

If something else needs the attribute right _between_ keypresses — a test
asserting on it, another script reading it — a `ResizeObserver` on the panel
...
The demo's markup carries its stylesheet, and it has one moving part. The panel
declares itself a container with `container-type: inline-size`; the grid inside
sets `--cols` and lays itself out with
...
navigation follows without a JavaScript edit — there is no second copy of the
numbers to keep in step.

The query measures the panel, not the viewport. Drop the same grid into a
sidebar and it gets the sidebar's column count, which a media query could not
give it. It is also why the container is the panel rather than the grid: an
element cannot query its own size.

The grid carries `data-keyrove-root` in the demo because the listener there sits
on the panel around it, the way a delegated listener does in
[several groups, one listener](/docs/installation#several-groups-one-listener).
With the listener on the grid itself, as in the snippet above, the attribute is
not needed.

## When the count is implicit

A grid built on `repeat(auto-fill, minmax(8rem, 1fr))` has no `--cols` to
read — the browser decides how many tracks fit. Count them instead: the
computed `grid-template-columns` of a grid container lists one size per column,
